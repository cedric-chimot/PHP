...
if (isset($_POST['submit'])) {
    ...
    $nom = htmlspecialchars($con, $_POST['nom']);
    $adresse = htmlspecialchars($con, $_POST['adresse']);
    $cp = htmlspecialchars($con, $_POST['cp']);
    $ville = htmlspecialchars($con, $_POST['ville']);
    $pays = htmlspecialchars($_POST['pays']);
    $email = htmlspecialchars($con, $_POST['email']);
    $password = htmlspecialchars($con, md5($_POST['password']));       //"Md5" calcul le hachage de la chaine de caractère
    $cpassword = htmlspecialchars($con, md5($_POST['cpassword']));
    ...
    if (mysqli_num_rows($select_user) > 0) {
        ...
        if ($password != $cpassword) {
            ...
            mysqli_query($con, "INSERT INTO `admin`(nom, adresse, cp, ville, pays, email, password, type_user)
