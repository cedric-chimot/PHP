...
if(isset($_POST['commander'])){
    ...
    if($total_panier == 0){
        ...
        if(mysqli_num_rows($commande_query) > 0){
            ...
            mysqli_query($con, "INSERT INTO `commandes`(idclient, nom, telephone, email, paiement,
                adresse, cp, ville, pays, total_articles, prix_total, date_commande, idvendeur)
                VALUES('$client_id', '$nom', '$telephone', '$email', '$paiement', '$adresse', '$cp', '$ville', '$pays',
                '$total_articles', '$total_panier', '$date_commande', '$vendeur_id')")
                or exit('Echec de la requête');
                $message[] = 'Commande effectuée !';
            //une fois la commande validée, le panier se vide
            mysqli_query($con, "DELETE FROM `panier` WHERE `idclient` = '$client_id'")
                or exit('Echec de la requête');
        }
    }
}
